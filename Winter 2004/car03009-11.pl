...
#seperate out the lengths of the file or directory names
   ...
   if( scalar @fourCharsLong )
   {
      ...
      print "<OPTION>Please Select a name</OPTION>\n";
      for $item ( @fourCharsLong )
      {
         if( ( $item ) && ( -f $item ) )
         {
            ...
            {
		$FoundItems{$fileContents[ 0 ]} = $item;
		#print "<OPTION VALUE='$item'>$fileContents[ 0 ]</OPTION>\n";
            }
         }
      }

      for $names ( sort keys %FoundItems )
      {
	  print "<OPTION VALUE=\"", $FoundItems{$names}, "\">$names</OPTION>\n";
      }

      print "</SELECT>\n";
      print "<INPUT id=\"Button1\" type=\"submit\" value=\"Show Transcript\" name=\"Button1\">\n";      
   }
   else
   {
